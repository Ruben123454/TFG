...
public:
    ...
    // Constructor para GPU
        __device__ Escenario()
                : primitivas(nullptr), num_primitivas(0), luces(nullptr), num_luces(0),
                    primitivas_malla(nullptr), num_primitivas_malla(0),
    ...
    __device__ Color calcularLuzDirecta(const Vector3d& punto, const Vector3d& normal,
        ...
        for (int i = 0; i < num_luces; i++) {
            ...
            float u, v;
            bool hit_sombra = false;
            if (tiny_bvh.d_nodos != nullptr && primitivas_tiny_bvh != nullptr && num_primitivas_tiny_bvh > 0) {
                hit_sombra = intersectaPrimitivasBVH_tiny(rayo_sombra, t_sombra, &objeto_sombra, u, v);
            } else {
                hit_sombra = intersectaPrimitivasBVH(rayo_sombra, t_sombra, &objeto_sombra, u, v);
            }
            if (hit_sombra) {
