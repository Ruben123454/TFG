...
public:
    ...
    TinyBVH(const TinyBVH&) = delete;
    TinyBVH& operator=(const TinyBVH&) = delete;
    TinyBVH(TinyBVH&&) = delete;
    TinyBVH& operator=(TinyBVH&&) = delete;

    void construirBVH(const std::vector<Primitiva>& primitivas_host);
    void obtenerInfo();
    TinyBVHD_GPU obtenerDatosGPU() const;
    const TinyBVHD_GPU* getNodosGPU() const;
    const Primitiva* getPrimitivasGPU() const;
    unsigned int getNumNodos() const;
    unsigned int getNumPrimitivas() const;
